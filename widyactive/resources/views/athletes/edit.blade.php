...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <!-- Breadcrumb / Header -->
    <div>
        <a href="{{ route('athletes.show', $athlete->id) }}" class="text-xs font-semibold text-slate-400 hover:text-white transition flex items-center space-x-1.5 mb-2">
            <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"/>
            </svg>
            <span>Back to Profile</span>
        </a>
        <h1 class="text-3xl font-extrabold tracking-tight text-white">Edit Athlete Details</h1>
        <p class="text-sm text-slate-400 mt-1">Update registration profiles for {{ $athlete->name }}</p>
    </div>
    ...
    <div class="glass-card p-6 md:p-8 border border-white/5 bg-slate-900/40 relative overflow-hidden">
        ...
        <form action="{{ route('athletes.update', $athlete->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="name" name="name" required value="{{ old('name', $athlete->name) }}" placeholder="e.g. John Doe"
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="email" id="email" name="email" required value="{{ old('email', $athlete->email) }}" placeholder="e.g. john@widyactive.com"
                           class="w-full px-4 py-2.5 rounded-lg bg-slate-950 border border-white/10 text-sm text-slate-200 outline-none focus:border-orange-500/50 transition">
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="date_of_birth" name="date_of_birth" required value="{{ old('date_of_birth', $athlete->date_of_birth->format('Y-m-d')) }}"
                           class="w-full px-4 py-2.5 rounded-lg bg-slate-950 border border-white/10 text-sm text-slate-200 outline-none focus:border-orange-500/50 transition">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="sport" name="sport" required value="{{ old('sport', $athlete->sport) }}" placeholder="e.g. Basketball, Running"
                           class="w-full px-4 py-2.5 rounded-lg bg-slate-950 border border-white/10 text-sm text-slate-200 outline-none focus:border-orange-500/50 transition">
                </div>
            </div>
            ...
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-white/5">
                <a href="{{ route('athletes.show', $athlete->id) }}" class="px-5 py-2.5 rounded-lg border border-white/10 bg-transparent text-sm font-semibold text-slate-300 hover:text-white transition">
                    Cancel
                </a>
